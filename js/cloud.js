/* Cloud sync — the transport only. Nothing in here knows what a task is; it
   ...
   That grant and those policies let anyone holding the URL and publishable
   key read and write the department's data. That is the trade for not running
   a login. A legacy anon key also works; secret and service_role keys must
   never be put in this browser app. */
...
async function explain(res) {
  ...
  if (res.status === 401 || res.status === 403) {
    return new Error(
      ...
      + 'key on an older project), the Data API grant and the three policies '
      + `from the setup SQL.${detail ? ` (${detail})` : ''}`);
  }
  if (res.status === 404) {
    return new Error(
...
export async function cloudTest(cfg) {
  if (!cfg?.url || !cfg?.key) {
    throw new Error('Both the project URL and the publishable key are needed.');
  }
  try {
    new URL(cfg.url);
  } catch {
